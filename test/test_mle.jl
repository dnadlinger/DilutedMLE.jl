...
function random_mle(projectors, asymptotic=true, shot_count=2^16 * length(projectors))
    ...
    ρest, converged, iters_taken = diluted_mle(projectors, weights, dense_identityoperator(b))

    return trnorm(ρ - ρest), converged, iters_taken, ρest
end

function test_mle(asymptotic, iter_count=1_000)
    trdists = Array{Float64}(iter_count)
    iters_takens = Array{Int64}(iter_count)
    ...
    for i = 1:iter_count
        ...
        projectors = [dense_identityoperator(b); [projector(s) for s in states]]
        two_qubit_projectors = vcat([[l ⊗ r for l in projectors] for r in projectors]...)
